...
function TOOL:LeftClick( trace )
	...
	if ( IsValid( trace.Entity ) and trace.Entity:GetClass() == "gmod_wire_joystick" and trace.Entity:GetTable().pl == ply ) then
...
function TOOL:UpdateGhostWirejoystick( ent, player )
	...
	if ( not trace.Hit or trace.Entity:IsPlayer() or trace.Entity:GetClass() == "gmod_wire_joystick" ) then
		ent:SetNoDraw( true )
		return
	end
...
function TOOL:Think()
	if ( not IsValid( self.GhostEntity ) or self.GhostEntity:GetModel() ~= self.Model ) then
